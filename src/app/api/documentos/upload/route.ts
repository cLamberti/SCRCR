...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!ALLOWED_TYPES.includes(file.type)) {
      return NextResponse.json({ success: false, message: 'Solo se permiten archivos PDF, JPG o PNG.' }, { status: 400 });
    }
    ...
    const safeFolder = ALLOWED_FOLDERS.includes(folder) ? folder : 'documentos';
    const timestamp = Date.now();
    const nombreLimpio = file.name.replace(/[^a-zA-Z0-9._-]/g, '_');
    const blobName = `${safeFolder}/${timestamp}_${nombreLimpio}`;
